...
def document(file_name):
    ...
    # load model 
    pkl_file = 'C:\\Users\\SRAVANTHI SHOROFF\\Desktop\\sravanthi\\SCOM_ML_v_2\\votingclassifierMod.pkl'
    ...
    # print(outcount['result'].value_counts())
    ...
    t_df = df1[df1.index.isin(dfnew_true.index)]
    t_df.reset_index(drop=True, inplace=True)
    a_df = df1[df1.index.isin(outcount.index)]
    # print(a_df)

    final = pd.concat([a_df,outcount],axis = 1)
    print(final)
    #Generate false alarm excel
    false_output = outcount[outcount['predicted_value']==0]
    ...
    true_output = outcount[outcount['predicted_value']==1]
